...
| `CARTESIA_API_KEY` | **Secret** | Optional once voice previews are synced to Storage; still used as preview fallback and by the sync script |
...
1. Go to **Business**.  
2. Enter name, category, contact, website, address, hours, services, policies, appointment rules, notification email (for post-call Resend delivery), optional WhatsApp destination (stored for later), and soft-handoff callback settings.  
