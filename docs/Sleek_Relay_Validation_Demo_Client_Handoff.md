...
| SMS / email close-off | Transactional SMS or email | **Notifications inbox** (post-call close-off log only) | Outbound WhatsApp / email deferred; see Notifications tab |
...
- Outbound transactional SMS / WhatsApp / Resend email delivery (Notifications inbox logs post-call close-off entries only)  
...
1. Go to **Business**.  
2. Enter name, category, contact, website, address, hours, services, policies, appointment rules, optional future notification destinations, and soft-handoff callback settings.  
