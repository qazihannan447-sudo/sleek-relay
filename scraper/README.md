...
| `GEMINI_API_KEY` / `GEMINI_MODEL` / `GEMINI_EMBEDDING_MODEL` | Local dev/demo only | Free-tier key, works but rate-limits hard under sustained multi-page use. Never use in production — see `llmClientGemini.ts`. |
